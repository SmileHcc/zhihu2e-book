...
class Init(object):
    ...
    def initDB(self):
        ...
        if os.path.isfile(databaseFile):
            ...
            cursor.execute("""create index idx_AnswerContent on
                AnswerContent(authorID, questionID, answerID, answerHref);""")
            ...
            cursor.execute("""
                            ...
                            primary key(collectionID, answerHref))""")

            # 话题内容表
            # 负责保存话题链接，每次获取话题内容时都要重新更新之
            cursor.execute("""
                            create  table       TopicIndex(
                            topicID             varchar(50)     not Null,
                            answerHref          varchar(255)    not Null,
                            primary key(topicID, answerHref))""")

            # 圆桌内容表
            # 负责保存圆桌内的答案链接，每次获取圆桌内容时都要重新更新之
            cursor.execute("""
                            create  table       TableIndex(
                            tableID             varchar(50)     not Null,
                            answerHref          varchar(255)    not Null,
                            primary key(tableID, answerHref))""")
